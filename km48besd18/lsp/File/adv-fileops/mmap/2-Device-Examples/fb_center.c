...
*/
pixel = ((red >> (8-var_info.red.length)) << var_info.red.offset) | ((green >> (8-var_info.green.length)) <<
var_info.green.offset) |((blue >>(8-var_info.blue.length)) << var_info.blue.offset);
}
else {
...
draw_pixel(x+1,y, pixel);
draw_pixel(x+2,y, pixel);
draw_pixel(x+3,y, pixel);
draw_pixel(x+4,y, pixel);
...
case 8:
	*((u_int8_t *)framebuffer + fix_info.line_length * y +x) = (u_int8_t)pixel;
break;
case 16:
	*((u_int16_t *)framebuffer + fix_info.line_length/2 *y +x) = (u_int16_t)pixel;
break;
case 32:
	*((u_int32_t *)framebuffer + fix_info.line_length/4*y +x) = (u_int32_t)pixel;
break;
default:
